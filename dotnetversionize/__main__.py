...
parser.add_argument("--release", help="Generate a release bump.", action="store_true")
parser.add_argument(
    "--release-title", nargs="?", help="Adds a title to a release, has no use otherwise"
)
parser.add_argument(
    "--release-candidate",
...
parser.add_argument(
    "--increment",
    choices=[
        IncrementType.MAJOR.name,
        IncrementType.MINOR.name,
        IncrementType.PATCH.name,
    ],
...
if __name__ == "__main__":
    ...
    if args.release or args.release_candidate:
        ...
        _increment_type = (
            IncrementType.detect_commit_list_increment(get_commit_list(_latest_tag))
            if not args.increment
            else IncrementType[args.increment]
        )
        ...
        _increment_type = (
            IncrementType.detect_commit_increment(get_commit_list("-1")[-1])
            if not args.increment
            else IncrementType[args.increment]
        )
        if _increment_type == IncrementType.NONE:
